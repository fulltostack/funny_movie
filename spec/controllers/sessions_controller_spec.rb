...
RSpec.describe SessionsController, type: :controller do
  ...
  describe 'POST #create' do
    context 'with correct credential' do
      it 'should login as an existing user' do
        ...
        validate_response(root_url, I18n.t('session.logged_in'), true, user.id)
      end
      ...
      it 'should register and login user if user does not exists' do
        ...
        validate_response(root_url, I18n.t('session.logged_in'), true, User.last.id)
      end
    end
    ...
    context 'with invalid credential' do
      it 'should not login with empty email' do
        ...
        validate_response(root_url, I18n.t('session.empty_required_fields'), true)
      end
      ...
      it 'should not login with invalid password' do
        ...
        validate_response(root_url, I18n.t('session.invalid_email_or_pwd'), true)
      end
      
      it 'should not login with invalid email format' do
        post :create, params: { 'email': 'abcd', 'password': Faker::Internet.password }
        validate_response(root_url, 'Error while creating user, please try again, error: Email is invalid', true)
      end
    end
  end
  ...
  describe 'DELETE #destroy' do
    it 'should logout a user' do
      ...
      delete :destroy
      validate_response(root_url, I18n.t('session.logged_out'), true)
    end
  end
end
